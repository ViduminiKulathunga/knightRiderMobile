...
import { setDriverStatus, setDrunkenOnTripLocal } from "../store/actions/data";

const ActiveService = (props) => {
  const [displayDrunkenMessage, setDisplayDrunkenMessage] = useState();
  const [displayLoginMessage, setDisplayLoginMessage] = useState(true);
  const [displayActiveMessage, setDisplayActiveMessage] = useState();
  const [status, setStatus] = useState("");
  const [tripPendingStatus, setTripPendingStatus] = useState(true);
  ...
  useEffect(() => {
    console.log(stateStatus, " stateStatus useEffect useEffect");
    ...
    if (!isCancelled) {
      if (stateStatus === 0) {
        ...
        console.log(tripPendingStatus, " tripPendingStatus EEEEE");
      } else {
        setTripPendingStatus(false);
        console.log(tripPendingStatus, " tripPendingStatus False Set RRRRR");
      }
    }
  ...
  useEffect(() => {
    ...
    if (!isCancelled) {
      ...
      getDrunkenStatus(rfidId).onSnapshot((doc) => {
        console.log(stateStatus, " stateStatus onSnapshot");

        const data = doc.data();

        //console.log("Single doc data ", data);
        if (data.status !== "logout" && !isCancelled) {
          console.log(data.status, " data.status data.status");
          setStatus("start");
          setLoading(true);
          console.log(
            " Changing setStatus Changing setStatus Changing setStatus"
          );
          ...
          if (drunken) {
            ...
            setTimeout(() => {
              console.log(
                tripPendingStatus,
                "tripPendingStatus Inside SetTimeOut"
              );
              ...
              console.log(
                drunkenHasPassenger,
                "drunkenHasPassenger Inside Drunken"
              );
              if (drunkenHasPassenger) {
                dispatch(setDriverStatus("drunkenHasPassenger"));
                dispatch(setDrunkenOnTripLocal(true));
                setStatus("drunkenHasPassenger");

                console.log("drunkenHasPassenger Common Common State");
                console.log(
                  tripPendingStatus,
                  "drunkenHasPassenger tripPendingStatus"
                );
                console.log("drunkenHasPassenger Common Common State");
              } else {
                setStatus("drunken");
      ...
      <Card style={styles.authContainer}>
        {isLoading ? (
          ...
          <View style={styles.UserView}>
            {displayDrunkenMessage &&
              (status === "drunken" || status === "drunkenHasPassenger") && (
                <View style={styles.textCenter}>
                  ...
                  <Text style={styles.warining}>
                    Please Tap Identity Card for Active your service in few
                    hours.
                  </Text>
                  <Text style={styles.warining}>
                    Please Try Few Hours Later..
                  </Text>
                </View>
              )}
